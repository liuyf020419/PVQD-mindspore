...
def np_cross(x, y):
    x_np = x.asnumpy()
    y_np = y.asnumpy()
    z = np.cross(x_np, y_np)
    return ms.Tensor(z, x.dtype)
...
def dist_ch(x1, x2, axis=2):
    d = x2-x1 + 1e-10
    d2 = ops.sum(d*d, dim=axis) # √(x2 + y2 + z2)
    return ops.sqrt(d2 + 1e-10)


def angle_ch(x1, x2, x3, degrees=True, axis=2):
    ba = x1 - x2 + 1e-10
    ba = ba / (ops.norm(ba, dim=axis, keepdim=True) + 1e-10)
    bc = x3 - x2 + 1e-10
    bc = bc / (ops.norm(bc, dim=axis, keepdim=True) + 1e-10)
    cosine_angle = ops.clip(ops.sum(ba*bc, dim=axis), min=-1+1e-6, max=1-1e-6)
    if degrees:
        return np.float32(180.0 / np.pi) * ops.acos(cosine_angle)
    else:
        return ops.acos(cosine_angle)
...
def torsion_ch(x1, x2, x3, x4, degrees=True, axis=2):
    ...
    b1 = b1 / (ops.norm(b1, dim=axis, keepdim=True) + 1e-10)
    ...
    v = b0 - ops.sum(b0*b1, dim=axis, keepdim=True) * b1
    w = b2 - ops.sum(b2*b1, dim=axis, keepdim=True) * b1

    # angle between v and w in a plane is the torsion angle
    # v and w may not be normalized but that's fine since tan is y/x
    x = ops.sum(v*w, dim=axis)
    b1xv = np_cross(b1, v)
    y = ops.sum(b1xv*w, dim=axis)
    # import pdb; pdb.set_trace()
    if degrees:
        return np.float32(180.0 / np.pi) * ops.atan2(y, x)
    else:
...
def preprocess_pair_feature(pair_feature):
    ...
    # import pdb; pdb.set_trace()
    CB_dist = ops.clamp(CB_dist, min=0, max=20) / 20
    omega_tor = ops.clamp(omega_tor, min=-np.pi, max=np.pi) / np.pi
    phi_ang = ops.clamp(phi_ang, min=0, max=np.pi) / np.pi
    theta_tor = ops.clamp(theta_tor, min=-np.pi, max=np.pi) / np.pi

    pair_feature = ops.cat([CB_dist[..., None], 
                              omega_tor[..., None], 
                              phi_ang[..., None], 
                              theta_tor[..., None]], -1)
    # import pdb; pdb.set_trace()
...
def rbf(D, num_rbf=16):
    ...
    D_expand = ops.unsqueeze(D, -1)
    RBF = ops.exp(-((D_expand - D_mu) / D_sigma)**2)
    return RBF


def triangle_encode_angles(angles: Tensor, tri_num=5):
    """
    angles: translate to triangle form
    k: cosine(k*(torsion))/sine(k*(torsion))
    """
    flatten_angles = angles.reshape(-1, 1)
    flatten_encoded_angles = ops.cat([ops.cat([ops.cos((k+1) * flatten_angles), 
                                                   ops.sin((k+1) * flatten_angles)], -1) 
...
def preprocess_pair_feature_advance(pair_feature, rbf_encode=True, num_rbf=16, tri_encode=True, tri_num=5):
    ...
    if rbf_encode:
        ...
        encoded_cb_dist = (ops.clamp(cb_dist, min=0, max=20) / 20)[..., None]
    if tri_encode:
        ...
        encoded_omega_tor = (ops.clamp(omega_tor, min=-np.pi, max=np.pi) / np.pi)[..., None]
        encoded_phi_ang = (ops.clamp(phi_ang, min=0, max=np.pi) / np.pi)[..., None]
        encoded_theta_tor = (ops.clamp(theta_tor, min=-np.pi, max=np.pi) / np.pi)[..., None]

    pair_feature = ops.cat([encoded_cb_dist, 
                              encoded_omega_tor, 
                              encoded_phi_ang, 
                              encoded_theta_tor], -1)
    return pair_feature
...
def calc_beta_dist(beta_coords):
    dist = ops.sqrt(ops.sum(
        ops.square(beta_coords[:, :, None] - beta_coords[:, None]), -1) + 1e-10)
    return dist
...
def generate_pair_from_pos(batch_pos, degrees=False):
    ...
    # import pdb; pdb.set_trace()
    pair_feature = ops.cat([CB_dist_map[..., None], 
...
def descrete_angle(angle, edges=None):
    ...
    angle = angle.int()
    angle = ops.clip(angle, 0, nbin_-1)
    return angle
...
def descrete_dist(dist, edges=None):
    ...
    dist = dist.astype(ms.int32)
    dist = ops.clip(dist, 0, nbin_-1)
    return dist
...
def get_internal_angles(batch_pos, degrees=False):
    ...
    phi_torsion = ops.pad(phi_torsion, (1, 0), "constant", 0)

    psi_torsion = torsion_ch(psi_n1, psi_ca, psi_c, psi_n2, degrees=degrees, axis=2)
    psi_torsion = ops.pad(psi_torsion, (0, 1), "constant", 0)

    internal_torsion = ops.stack([phi_torsion, psi_torsion], -1)
...
def get_internal_angles3(batch_pos, degrees=False):
    ...
    omega_torsion = ops.pad(omega_torsion, (0, 1), "constant", 0)

    internal_torsion = ops.stack([phi_torsion, psi_torsion, omega_torsion], -1)
...
def get_descrete_dist(all_atoms, dist_type, distogram_args, return_dist_map=False):
    ...
    dist_map = ops.sqrt(ops.sum(ops.square(atom1_coord[:, None] - atom2_coord[:, :, None]), -1) + 1e-10)
    dist_map_descrete = descrete_dist(dist_map, distogram_args).float()
...
def get_descrete_feature(all_atoms, return_angle_mask=True, mask_base_ca=True, mask_cutoff=10):
    ...
    ca_dist_map = ops.sqrt(ops.sum(ops.square(ca_coord[:, None] - ca_coord[:, :, None]), -1) + 1e-10)
