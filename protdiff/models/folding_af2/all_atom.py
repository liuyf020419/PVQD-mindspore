...
restype_rigidgroup_base_atom37_idx_ = restype_rigidgroup_base_atom37_idx
restype_rigidgroup_mask_ = restype_rigidgroup_mask

restype_rigidgroup_is_ambiguous_ = np.zeros([21, 8])
restype_rigidgroup_rots_ = np.tile(np.eye(3, dtype=np.float32), [21, 8, 1, 1])
...
def get_chi_atom_indices():
  ...
  return Tensor(chi_atom_indices)
...
def atom14_to_atom37(atom14_data,  # (N, 14, ...)
  ...
  atom37_data = ops.gather_elements(atom14_data, 1, batch['residx_atom37_to_atom14'].unsqueeze(-1).repeat(1, 1, 3))
...
def atom37_to_atom14(
  ...
  atom14_data = ops.gather_elements(atom37_data, 1, gather_idx)
...
def atom37_to_frames(
  ...
  # shape (N, 8, 3)
  restype_rigidgroup_base_atom37_idx = restype_rigidgroup_base_atom37_idx_
  ...
  gather_idx = np.expand_dims(residx_rigidgroup_base_atom37_idx.reshape(aatype.shape[0], -1), axis=-1)
  gather_idx = gather_idx.repeat(3, axis=2)
  base_atom_pos = np.take_along_axis(all_atom_positions, gather_idx, axis=1)
  # [nres, frames, 3 atomes, xyz]
  base_atom_pos = base_atom_pos.reshape(aatype.shape[0], 8, 3, 3)
  ...
  #     residx_rigidgroup_base_atom37_idx,
  #     batch_dims=1)
  gather_idx = residx_rigidgroup_base_atom37_idx.reshape(aatype.shape[0], -1).astype(np.int32)
  gt_atoms_exist = np.take_along_axis(all_atom_mask, gather_idx, axis=1)
  gt_atoms_exist = gt_atoms_exist.reshape(aatype.shape[0], 8, 3)
  gt_exists = np.min(gt_atoms_exist, axis=-1)[0] * group_exists  # (N, 8)
  ...
  restype_rigidgroup_is_ambiguous = restype_rigidgroup_is_ambiguous_
  restype_rigidgroup_rots = restype_rigidgroup_rots_
  ...
  gt_frames_flat12 = gt_frames_flat12.reshape(aatype_in_shape + (8, 12))
    # jnp.reshape(gt_frames_flat12, aatype_in_shape + (8, 12))
  gt_exists = gt_exists.reshape(aatype_in_shape + (8,))
    # jnp.reshape(gt_exists, aatype_in_shape + (8,))
  group_exists = group_exists.reshape(aatype_in_shape + (8,))
    # jnp.reshape(group_exists, aatype_in_shape + (8,))
  gt_frames_flat12 = gt_frames_flat12.reshape(aatype_in_shape + (8, 12))
    # jnp.reshape(gt_frames_flat12, aatype_in_shape + (8, 12))
  residx_rigidgroup_is_ambiguous = residx_rigidgroup_is_ambiguous.reshape(aatype_in_shape + (8,))
    # jnp.reshape(residx_rigidgroup_is_ambiguous,
    #                                            aatype_in_shape + (8,))
  alt_gt_frames_flat12 = alt_gt_frames_flat12.reshape(aatype_in_shape + (8, 12,))
...
def atom37_to_torsion_angles(
    aatype: Tensor,  # (B, N)
    all_atom_pos: Tensor,  # (B, N, 37, 3)
    all_atom_mask: Tensor,  # (B, N, 37)
  ...
  pad = ops.zeros((num_batch, 1, 37, 3)).float()
  # all previous atoms
  prev_all_atom_pos = ops.cat([pad, all_atom_pos[:, :-1, :, :]], axis=1)

  pad = ops.zeros((num_batch, 1, 37)).float()
  prev_all_atom_mask = ops.cat([pad, all_atom_mask[:, :-1, :]], axis=1)
  ...
  # Shape [batch, num_res]
  pre_omega_mask = (
      ops.prod(prev_all_atom_mask[:, :, 1:3], axis=-1)  # prev CA, C
      * ops.prod(all_atom_mask[:, :, 0:2], axis=-1))  # this N, CA
  phi_mask = (
      prev_all_atom_mask[:, :, 2]  # prev C
      * ops.prod(all_atom_mask[:, :, 0:3], axis=-1))  # this N, CA, C
  psi_mask = (
      ops.prod(all_atom_mask[:, :, 0:3], axis=-1) *  # this N, CA, C
  ...
  chis_mask = ops.gather_elements(chi_angles_mask, 0, indices).view(B, N, 4)
  ...
  torsion_angles_sin_cos = ops.stack(
      [forth_atom_rel_pos.z, forth_atom_rel_pos.y], axis=-1)
  torsion_angles_sin_cos /= ops.sqrt(
      ops.sum(ops.square(torsion_angles_sin_cos), dim=-1, keepdims=True)
      + 1e-8)

  # Mirror psi, because we computed it from the Oxygen-atom. ??? L-amino acid
  torsion_angles_sin_cos = torsion_angles_sin_cos * Tensor(
      [1., 1., -1., 1., 1., 1., 1.])[None, None, :, None]

  # Create alternative angles for ambiguous atom names.
  # get indices for posible residue
  chi_pi_periodic = Tensor(residue_constants.chi_pi_periodic)
  indeces = aatype.unsqueeze(-1).repeat(1, 1, 4).view(-1, 4)
  chi_is_ambiguous = ops.gather_elements(chi_pi_periodic, 0, indeces).view(B, N, 4)

  mirror_torsion_angles = ops.cat(
      [ops.ones((num_batch, num_res, 3)),
  ...
  if placeholder_for_undefined:
    ...
    placeholder_torsions = ops.stack([
        ops.ones(torsion_angles_sin_cos.shape[:-1]),
        ops.zeros(torsion_angles_sin_cos.shape[:-1])
    ], axis=-1)
...
def torsion_angles_to_frames(
  ...
  m = ops.gather_elements(residue_constants.restype_rigid_group_default_frame, 0,
  ...
  sin_angles = ops.cat([ops.zeros((num_residues, 1)), sin_angles],
                               axis=-1)
  cos_angles = ops.cat([ops.ones((num_residues, 1)), cos_angles],
                               axis=-1)
  zeros = ops.zeros_like(sin_angles)
  ones = ops.ones_like(sin_angles)
...
def frames_and_literature_positions_to_atom14_pos(
  ...
  residx_to_group_idx = ops.gather_elements(residue_constants.restype_atom14_to_rigid_group, 0, 
                                     aatype.unsqueeze(-1).repeat(1, 14) )
  # one-hot tranformation
  group_mask = ops.one_hot(
      residx_to_group_idx, depth=8)  # shape (N, 14, 8)
  ...
  map_atoms_to_global = r3.tree_map_rigids(
      lambda x: ops.sum(x[:, None, :] * group_mask, dim=-1),
  ...
  lit_positions = r3.vecs_from_tensor(
    ops.gather_elements(residue_constants.restype_atom14_rigid_group_positions, 0, 
  ...
  mask = ops.gather_elements(residue_constants.restype_atom14_mask, 0, 
                                     aatype.unsqueeze(-1).repeat(1, 14) )
...
def extreme_ca_ca_distance_violations(
  ...
  ca_ca_distance = ops.sqrt(
      1e-6 + ops.sum(squared_difference(this_ca_pos, next_ca_pos), dim=-1))
...
def between_residue_bond_loss_batch(
  ...
  c_n_bond_length = ops.sqrt(
      1e-6 + ops.sum(squared_difference(this_c_pos, next_n_pos), dim=-1))
  ...
  c_n_bond_length_error = ops.sqrt(1e-6 +
                                   ops.square(c_n_bond_length - gt_length))
  c_n_loss_per_residue = ops.relu(
      c_n_bond_length_error - tolerance_factor_soft * gt_stddev) * weight
  # import pdb; pdb.set_trace()
  mask = this_c_mask * next_n_mask * has_no_gap_mask
  # c_n_loss = ops.sum(mask * c_n_loss_per_residue, 1) / (ops.sum(mask, 1) + 1e-6)
  c_n_loss = sum(ops.sum(mask * c_n_loss_per_residue, 1) / (ops.sum(mask, 1) + 1e-6)) / mask.shape[0]
  c_n_violation_mask = mask * (
      c_n_bond_length_error > (tolerance_factor_hard * gt_stddev))

  # Compute loss for the angles.
  ca_c_bond_length = ops.sqrt(1e-6 + ops.sum(
      squared_difference(this_ca_pos, this_c_pos), dim=-1))
  n_ca_bond_length = ops.sqrt(1e-6 + ops.sum(
  ...
  ca_c_n_cos_angle = ops.sum(c_ca_unit_vec * c_n_unit_vec, dim=-1)
  gt_angle = residue_constants.between_res_cos_angles_ca_c_n[0]
  gt_stddev = residue_constants.between_res_bond_length_stddev_c_n[0]
  ca_c_n_cos_angle_error = ops.sqrt(
      1e-6 + ops.square(ca_c_n_cos_angle - gt_angle))
  ca_c_n_loss_per_residue = ops.relu(
      ca_c_n_cos_angle_error - tolerance_factor_soft * gt_stddev) * weight
  # import pdb; pdb.set_trace()
  mask = this_ca_mask * this_c_mask * next_n_mask * has_no_gap_mask
  # ca_c_n_loss = ops.sum(mask * ca_c_n_loss_per_residue) / (ops.sum(mask) + 1e-6)
  ca_c_n_loss = sum(ops.sum(mask * ca_c_n_loss_per_residue, 1) / (ops.sum(mask, 1) + 1e-6)) / mask.shape[0]
  ca_c_n_violation_mask = mask * (ca_c_n_cos_angle_error >
                                  (tolerance_factor_hard * gt_stddev))

  c_n_ca_cos_angle = ops.sum((-c_n_unit_vec) * n_ca_unit_vec, dim=-1)
  gt_angle = residue_constants.between_res_cos_angles_c_n_ca[0]
  gt_stddev = residue_constants.between_res_cos_angles_c_n_ca[1]
  c_n_ca_cos_angle_error = ops.sqrt(
      1e-6 + ops.square(c_n_ca_cos_angle - gt_angle))
  c_n_ca_loss_per_residue = ops.relu(c_n_ca_cos_angle_error - tolerance_factor_soft * gt_stddev) * weight
  mask = this_c_mask * next_n_mask * next_ca_mask * has_no_gap_mask
  # c_n_ca_loss = ops.sum(mask * c_n_ca_loss_per_residue) / (ops.sum(mask) + 1e-6)
  c_n_ca_loss = sum(ops.sum(mask * c_n_ca_loss_per_residue, 1) / (ops.sum(mask, 1) + 1e-6)) / mask.shape[0]
  ...
  per_residue_loss_sum = 0.5 * (ops.pad(per_residue_loss_sum, (0, 1)) + ops.pad(per_residue_loss_sum, (1, 0)))
  # Compute hard violations.
  violation_mask = ops.max(
      ops.stack([c_n_violation_mask,ca_c_n_violation_mask,c_n_ca_violation_mask], 1), axis=1)[0]
  violation_mask = ops.max(ops.stack(
      [ops.pad(violation_mask, (0, 1)),
      ops.pad(violation_mask, (1, 0))], 1), axis=1)[0]
...
def between_residue_bond_loss(
  ...
  # import pdb; pdb.set_trace()
  mask = this_c_mask * next_n_mask * has_no_gap_mask
  c_n_loss = ops.sum(mask * c_n_loss_per_residue) / (ops.sum(mask) + 1e-6)
  c_n_violation_mask = mask * (
      c_n_bond_length_error > (tolerance_factor_hard * gt_stddev))
  ...
  c_n_ca_cos_angle_error = ops.sqrt(
      1e-6 + ops.square(c_n_ca_cos_angle - gt_angle))
  c_n_ca_loss_per_residue = ops.relu(
  ...
  per_residue_loss_sum = (c_n_loss_per_residue +
                          ca_c_n_loss_per_residue +
                          c_n_ca_loss_per_residue)
  per_residue_loss_sum = 0.5 * (ops.pad(per_residue_loss_sum, (0, 1)) +
                                ops.pad(per_residue_loss_sum, (1, 0)))
  # Compute hard violations.
  violation_mask = ops.max(
      ops.stack([c_n_violation_mask,
                 ca_c_n_violation_mask,
                 c_n_ca_violation_mask]), axis=0)[0]
  violation_mask = ops.max(ops.stack(
      [ops.pad(violation_mask, (0, 1)),
      ops.pad(violation_mask, (1, 0))]), axis=0)[0]
...
def between_residue_clash_loss_batch(
  ...
  c_one_hot = ops.one_hot(Tensor([2]), natoms, 1, 0).squeeze(0)
  n_one_hot = ops.one_hot(Tensor([0]), natoms, 1, 0).squeeze(0)
  ...
  # cys_sg_one_hot = ops.one_hot(Tensor([cys_sg_idx]), depth=natoms).squeeze(0)
  ...
  # Compute the error.
  # shape (B, N, N, 14, 14)
  dists_to_low_error = dists_mask * ops.relu(
      dists_lower_bound - overlap_tolerance_soft - dists)

  # Compute the mean loss.
  # shape ()
  mean_loss = (ops.sum(dists_to_low_error)
               / (1e-6 + ops.sum(dists_mask)))

  # Compute the per atom loss sum.
  # shape (B, N, 14)
  per_atom_loss_sum = (ops.sum(dists_to_low_error, dim=[1, 3]) +
                       ops.sum(dists_to_low_error, dim=[2, 4]))
  ...
  # shape (B, N, 14)
  per_atom_clash_mask = ops.stack([clash_mask.max(3)[0].max(1)[0], clash_mask.max(4)[0].max(1)[0]], 1).max(1)[0]
...
def between_residue_clash_loss(
  ...
  # Backbone C--N bond between subsequent residues is no clash.
  c_one_hot = ops.one_hot(Tensor([2]), depth=natoms).squeeze(0)
  n_one_hot = ops.one_hot(Tensor([0]), depth=natoms).squeeze(0)
  neighbour_mask = ((residue_index[:, None, None, None] +
                     1) == residue_index[None, :, None, None])
  ...
  # Compute the error.
  # shape (N, N, 14, 14)
  dists_to_low_error = dists_mask * ops.relu(dists_lower_bound - overlap_tolerance_soft - dists)
  ...
  # Compute the per atom loss sum.
  # shape (N, 14)
  per_atom_loss_sum = (ops.sum(dists_to_low_error, dim=[0, 2]) +
                       ops.sum(dists_to_low_error, dim=[1, 3]))
  ...
  # Compute the per atom clash.
  # shape (N, 14)
  per_atom_clash_mask = ops.stack([clash_mask.max(2)[0].max(0)[0], clash_mask.max(3)[0].max(0)[0]]).max(0)[0]
...
def within_residue_violations_batch(
  ...
  # shape (B, N, 14, 14)
  dists_masks = (1. - ops.eye(natoms, natoms)[None, None])
  ...
  # shape (B, N, 14)
  per_atom_violations = ops.stack([violations.max(2)[0], violations.max(3)[0]], 1).max(1)[0]
...
def within_residue_violations(
  ...
  # Compute the mask for each residue.
  # shape (N, 14, 14)
  dists_masks = (1. - ops.eye(natoms, natoms)[None])
  dists_masks = dists_masks * (atom14_atom_exists[:, :, None] *
                  atom14_atom_exists[:, None, :])
  ...
  # Compute the per atom loss sum.
  # shape (N, 14)
  per_atom_loss_sum = (ops.sum(loss, dim=1) +
                       ops.sum(loss, dim=2))
  ...
  # Compute the per atom violations.
  # shape (N, 14)
  per_atom_violations = ops.stack([violations.max(1)[0], violations.max(2)[0]]).max(0)[0]
...
def find_optimal_renaming(
  ...
  # shape (N, N, 14, 14)
  pred_dists = ops.sqrt(1e-10 + ops.sum(
      squared_difference(
          atom14_pred_positions[:, None, :, None, :],
          atom14_pred_positions[None, :, None, :, :]),
      dim=-1))
  ...
  # shape (N, N, 14, 14)
  gt_dists = ops.sqrt(1e-10 + ops.sum(
      squared_difference(
          atom14_gt_positions[:, None, :, None, :],
          atom14_gt_positions[None, :, None, :, :]),
      dim=-1))
  alt_gt_dists = ops.sqrt(1e-10 + ops.sum(
      squared_difference(
  ...
  # Compute LDDT's.
  # shape (N, N, 14, 14)
  lddt = ops.sqrt(1e-10 + squared_difference(pred_dists, gt_dists))
  alt_lddt = ops.sqrt(1e-10 + squared_difference(pred_dists, alt_gt_dists))
  ...
  # shape (N)
  per_res_lddt = ops.sum(mask * lddt, dim=[1, 2, 3])
  alt_per_res_lddt = ops.sum(mask * alt_lddt, dim=[1, 2, 3])
...
def frame_aligned_point_error(
  ...
  normalization_factor = (
      ops.sum(frames_mask.float(), dim=-1) *
      ops.sum(positions_mask.float(), dim=-1))
  return (ops.sum(normed_error, dim=(-2, -1)) /
...
def get_alt_atom14(aatype, positions, mask):
  ...
  # shape (num_res, 14, 14)
  renaming_transform = ops.gather_elements(Tensor(RENAMING_MATRICES, 0, aatype))

  positions = r3.tree_map_vecs(lambda x: x[:, :, None], positions)
  alternative_positions = r3.tree_map_vecs(
      lambda x: ops.sum(x, dim=1), positions * renaming_transform)

  # Create the mask for the alternative ground truth (differs from the
  # ground truth mask, if only one of the atoms in an ambiguous pair has a
  # ground truth position)
  alternative_mask = ops.sum(mask[..., None] * renaming_transform, dim=1)
