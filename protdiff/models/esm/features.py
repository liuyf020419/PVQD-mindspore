...
class GVPInputFeaturizer(nn.Cell):
    ...
    def _dihedrals(x, eps=1e-7):
        ...
        # Backbone normals
        n_2 = normalize(np_cross(u_2, u_1), dim=-1)
        n_1 = normalize(np_cross(u_1, u_0), dim=-1)
    ...
    def _positional_embeddings(edge_index,
        ...
        if res_idx is not None:
            edge_start = ops.gather_elements(res_idx, -1, edge_index[0])
            edge_end = ops.gather_elements(res_idx, -1, edge_index[1])
        else:
        ...
        min_dist = int(0-clamp_dist)
        max_dist = int(clamp_dist)
        d = ops.clamp(d, min=min_dist, max=max_dist)
    ...
    @staticmethod
    def _dist(x, coord_mask, res_idx, padding_mask, top_k_neighbors, order=3):
        ...
        d_adjust = nan_to_num(D_cov) + (verse_coord_mask_2d) * (1e8 + d_seq * 1e6) + (
            verse_residue_mask_2d) * (1e10)
        d_adjust = d_adjust.astype(ms.float32)
        if top_k_neighbors == -1:
            ...
            k = min(top_k_neighbors, maxlen)
            d_neighbors, e_idx = ops.topk(d_adjust, k, dim=-1, largest=False)
            d_neighbors = ops.gather_elements(d, -1, e_idx)

        d_neighbors = ms.Tensor(d_neighbors, ms.float32)
...
class DihedralFeatures(nn.Cell):
    ...
    def _dihedrals(self, x, eps=1e-7, ):
        ...
        u_0 = u[:, 2:, :]
        # Backbone normals
        n_2 = ops.L2Normalize(axis=-1)(np_cross(u_2, u_1))
        n_1 = ops.L2Normalize(axis=-1)(np_cross(u_1, u_0))
        ...
        if self.node_angle_len >1:
            select_idx = ops.arange(0, res_num, self.node_angle_len)
            expanded_D = d.reshape(batchsize, len(select_idx), -1)

            D_features = ops.cat((ops.cos(expanded_D), ops.sin(expanded_D)), -1)
        else:
            D_features = ops.cat([ops.cos(d), ops.sin(d)], -1)
        return D_features


    def construct(self, x, torsion=None):
        """ Featurize coordinates as an attributed graph """
        if torsion is not None:
            v = ops.cat(
                    [ops.sin(torsion), ops.cos(torsion)], axis=-1
                )
        else:
            v = self._dihedrals(x)
...
class GVPGraphEmbedding(GVPInputFeaturizer):
    ...
    def __init__(self, args):
        ...
        self.clamp_rel_pos = int(args.clamp_rel_pos)
    ...
    def get_edge_features(self, coords, coord_mask, res_idx, padding_mask, edge_cross_vec=False):
        ...
        if edge_cross_vec:
            sidechains =  GVPInputFeaturizer._sidechains(coords)
            sc_src = ops.flatten(sidechains.unsqueeze(2).broadcast_to((-1, -1, k, -1)), start_dim=1, end_dim=2)
            sc_dest = ops.gather_elements(
                sidechains,
                1,
                edge_index[1, :, :].unsqueeze(-1).broadcast_to((e_idx_b, e_idx_l*k, 3))
            )
            sc_cross_vectors = np_cross(sc_src, sc_dest)
            sc_cross_vectors_mean = ops.sum(sc_cross_vectors * e_coord_mask, dim=1,
                keepdim=True) / ops.sum(e_coord_mask, dim=1, keepdim=True)
            ...
            edge_v = ops.cat([edge_v, edge_sc_v], -2)
